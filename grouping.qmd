...
judgments_condition_stats <- summarise(judgments,
          mean = mean(moral_dilemma_dog, na.rm= TRUE),
          med = median(moral_dilemma_dog, na.rm= TRUE),
          std = sd(moral_dilemma_dog, na.rm= TRUE),
          min = min(moral_dilemma_dog, na.rm = TRUE),
          max = max(moral_dilemma_dog, na.rm = TRUE))
```

```{r}
judgments |> 
  group_by(condition) |> 
  summarise(mean = mean(moral_dilemma_dog, na.rm= TRUE),
          med = median(moral_dilemma_dog, na.rm= TRUE),
          std = sd(moral_dilemma_dog, na.rm= TRUE),
          min = min(moral_dilemma_dog, na.rm = TRUE),
          max = max(moral_dilemma_dog, na.rm = TRUE))
...
"There are fourteen 20 years of age females in the `stress` group."
...
```{r}
# Write your answer here
count(judgments, gender, condition, age,  sort = TRUE)
```
